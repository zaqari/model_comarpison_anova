...
class MANOVA():
    ...
    # (1) Drop duplicates for group_columns to get the total number of groups
    # (2) Use the group combinations to create alpha values and index them.

    def __init__(self, df: pd.DataFrame, group_columns: List[str], y_column: str):
        super(MANOVA,self).__init__()
        print('THIS MODEL IS NOT COMPLETE. DO NOT USE IT.')
        self.group = group_columns
        self.Y = y_column
        self.df = df
        self.df['e'] = None
